...
func (downloader HttpDownloader) Download(file *HttpFile) error {
	...
	console.Debug("下载: %s -> %s", file.Url, file.Name)
	resp, err := http.Get(file.Url)
	if err != nil {
		console.Error("下载 %s 失败, %s", file.Url, err)
	}
	defer resp.Body.Close()
	console.Debug("size: %v", resp.Header.Get("Content-Length"))
	...
	console.Debug("下载 %s 完成", file.Url)
	wt.Flush()
	return nil
}
